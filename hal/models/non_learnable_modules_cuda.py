...
class NonLearnableLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        x_conv = self.conv(x)
        d_x1,d_x2,d_x3 = nl_module_cuda.nl_forward(x,x_conv,poolsize,stride,padding)
        ...
        x2 = self.maxpool(x)
        x3 = self.avgpool(x)
        print(f"x1 size: {x1.shape}, d_x1 size: {d_x1.shape}")
        print(f"x2 size: {x2.shape}, d_x2 size: {d_x2.shape}")
        print(f"x3 size: {x3.shape}, d_x3 size: {d_x3.shape}")
        input("Press Enter to continue...")
