...
std::vector<torch::Tensor> nl_forward_withcat(
    torch::Tensor input,
    torch::Tensor conv_input,
    torch::Tensor poolsize,
    torch::Tensor stride,
    ...
    return nl_forward_withcat_cuda(input, conv_input, poolsize, stride, padding);
}

PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  m.def("nl_forward", &nl_forward, "nl forward (CUDA)");
  m.def("nl_forward_withcat", &nl_forward_withcat, "nl forward with cat (CUDA)");
