...
// max pooling with padding test kernel version 1
template <typename scalar_t>
__global__ void nl_maxpooling_withpadding_kernel_v1(
    const torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> input,
    const int pw, const int ph, const int stride_x, const int stride_y, const int padx, const int pady,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> output) {
  //batch index
  int n = blockIdx.x;
  //channel index
  ...
  int h = threadIdx.z;
  //iteration counter
  int ii, jj, istart, iend, jstart, jend;
  //max pooling with padding
  if (n < output.size(0) && c < output.size(1) && w < output.size(2) && h < output.size(3)){
    // if the pooling region only contains padding values, set output value to 0
    if (w*stride_x+pw < padx || h*stride_y > input.size(2)+padx-1 ||
        h*stride_y+ph < pady || h*stride_y > input.size(3)+pady-1){
      output[n][c][w][h] = 0.0;
    }else{
      ...
      if (w*stride_x < padx){
        ...
        istart = (w-padx)*stride_x;
      }
      // vertical start index
      if (h*stride_y < pady){
        jstart = 0.0;
      }else{
        jstart = (h-pady)*stride_y;
      }
      // horizontal end index
      if (w*stride_x+pw > input.size(2)+padx-1){
        iend = input.size(2);
      }else{
        iend = (w-padx)*stride_x+pw;
      }
      // vertical end index
      if (h*stride_y+ph > input.size(3)+pady-1){
        jend = input.size(3);
      }else{
        jend = (h-pady)*stride_y+ph;
      }
      //printf("(w,h)=(%d,%d)\n",w,h);
      //printf("(istart,iend,jstart,jend)=(%d,%d,%d,%d)\n",istart,iend,jstart,jend);
      //initialize pooling
      output[n][c][w][h] = input[n][c][istart][jstart];
      //start pooling
...
// max pooling with padding test kernel wrapper function v1
...
std::vector<torch::Tensor> nl_maxpooling_withpadding_cuda_v1(
  ...
  torch::Tensor padding){
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "nl_maxpool_padding_gpu_v1", ([&] {
      nl_maxpooling_withpadding_kernel_v1<scalar_t><<<num_batch, block>>>(
          input.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
...
// average pooling with padding test kernel
template <typename scalar_t>
__global__ void nl_avgpooling_withpadding_kernel_v1(
    const torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> input,
    const int pw, const int ph, const int stride_x, const int stride_y, const int padx, const int pady,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> output) {
  //batch index
...
std::vector<torch::Tensor> nl_avgpooling_withpadding_cuda_v1(
  ...
  torch::Tensor padding){
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "nl_avgpool_padding_gpu_v1", ([&] {
      nl_avgpooling_withpadding_kernel_v1<scalar_t><<<num_batch, block>>>(
          input.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
          poolsize_a[0], poolsize_a[1], stride_a[0], stride_a[1], padding_a[0], padding_a[1],
          output.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>());
    }));
...
std::vector<torch::Tensor> nl_forward_cuda(
  ...
  // forward kernel function v1
template <typename scalar_t>
__global__ void nl_forward_kernel_v1(
    const torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> input,
    const torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> conv_input,
    const int pw, const int ph, const int stride_x, const int stride_y, const int padx, const int pady,
    torch::PackedTensorAccessor32<scalar_t,4,torch::RestrictPtrTraits> output1,
  ...
  if (n < output2.size(0) && c < output2.size(1) && w < output2.size(2) && h < output2.size(3)){
    ...
    if (w*stride_x+pw < padx || h*stride_y > input.size(2)+padx-1 ||
      ...
      //initialize pooling
      output2[n][c][w][h] = input[n][c][istart][jstart];
      for( ii = istart; ii < iend; ii++){
        for( jj = jstart; jj < jend; jj++){
          if(input[n][c][ii][jj] > output2[n][c][w][h]){
            output2[n][c][w][h] = input[n][c][ii][jj];
          }
        }
      }
    }
  ...
  if (n < output3.size(0) && c < output3.size(1) && w < output3.size(2) && h < output3.size(3)){
    ...
    if (w*stride_x+pw < padx || h*stride_y > input.size(2)+padx-1 ||
      ...
      for( ii = istart; ii < iend; ii++){
        for( jj = jstart; jj < jend; jj++){
          output3[n][c][w][h] += input[n][c][ii][jj];
        }
      }
      output3[n][c][w][h] = output3[n][c][w][h]/(pw*ph);
    }
...
std::vector<torch::Tensor> nl_forward_cuda_v1(
  ...
  torch::Tensor padding){
    ...
    AT_DISPATCH_FLOATING_TYPES(input.type(), "nl_forward_gpu_v1", ([&] {
      nl_forward_kernel_v1<scalar_t><<<num_batch, block>>>(
          input.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
          conv_input.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
          poolsize_a[0], poolsize_a[1], stride_a[0], stride_a[1], padding_a[0], padding_a[1],
          output1.packed_accessor32<scalar_t,4,torch::RestrictPtrTraits>(),
